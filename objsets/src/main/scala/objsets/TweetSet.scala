...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    right.filterAcc(p, left.filterAcc(p, filtered))
  }
  ...
  def mostRetweeted: Tweet = {
    val lMost = if (!left.isInstanceOf[Empty]) left.mostRetweeted else elem
    val rMost = if (!right.isInstanceOf[Empty]) right.mostRetweeted else elem

    val Most = if (lMost.retweets > elem.retweets) lMost else elem
    if (rMost.retweets > Most.retweets) rMost else Most
  }
