...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    response.foreach{
      chunk =>
        if (!response.hasNext && token.isCancelled) return
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val listenerSubs = httpListener.start()
    val responderSubs = Future.run(){
    cancelToken =>
      Future{
        async{
          while (!cancelToken.isCancelled) {
            val (req, xchg) = await(httpListener.nextRequest())
            respond(xchg, cancelToken, handler(req))
          }
        }
      }
    }
    Subscription(listenerSubs, responderSubs)
  }
