...
//              if (!Promise[List[T]]().isCompleted) Promise[List[T]]().failure(exception)
//            case Success(value) =>
//              res += value
//          }
//          rem-=1
//          if (!Promise[List[T]]().isCompleted && rem == 0) Promise[List[T]]().success(res.toList)
//      }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      fs.foreach{
        f =>
          f onComplete{
            case res =>
              if (!Promise[T].isCompleted) Promise[T].complete(res)
          }
      }
      Promise[T]().future
    }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      val future = f(token.cancellationToken)

      new Subscription {
        override def unsubscribe(): Unit = token.unsubscribe()
      }
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      f onComplete{
        case _ => Promise[S]().success(cont(f))
      }
      Promise[S]().future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      f onComplete {
        case result => Promise[S]().success(cont(result))
      }
      Promise[S]().future
    }
  }
